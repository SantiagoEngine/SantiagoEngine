...
class TitleState extends MusicBeatState
{
	...
	override public function create():Void
	{
		...
		if(ClientPrefs.data.checkForUpdates && !closedState) {
			...
			var http = new haxe.Http("https://raw.githubusercontent.com/SantiagoEngine/SantiagoEngine/main/gitVersion.txt");
	...
	function startIntro()
	{
		...
		seteam = new FlxSprite(0, FlxG.height * 0.52).loadGraphic(Paths.image('seteam'));
		add(seteam);
		seteam.visible = false;
		seteam.setGraphicSize(Std.int(seteam.width * 0.8));
		seteam.updateHitbox();
		seteam.screenCenter(X);
		seteam.antialiasing = ClientPrefs.data.antialiasing;
	...
	override function beatHit()
	{
		...
		FlxG.camera.zoom += 0.030; //zoom yeah
		...
		if(!closedState) {
			...
			switch (sickBeats)
			{
				...
				case 2:
					createCoolText(['Santiago Engine by:']);
				case 4:
					addMoreText('SantiagoEngine Team');
					seteam.visible = true;
				case 5:
					seteam.visible = false;
					deleteCoolText();
				case 6:
					createCoolText(['Friday Night Funkin\' with'], -40);
				case 8:
	...
	function skipIntro():Void
	{
		if (!skippedIntro)
		{
			...
			logoBl.angle = -4;  //logo shit yeah

				new FlxTimer().start(0.01, function(tmr:FlxTimer)
				{
					if (logoBl.angle == -4)
						FlxTween.angle(logoBl, logoBl.angle, 4, 4, {ease: FlxEase.expoInOut});
					if (logoBl.angle == 4)
						FlxTween.angle(logoBl, logoBl.angle, -4, 4, {ease: FlxEase.expoInOut});
				}, 0);

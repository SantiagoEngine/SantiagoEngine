...
class PlayState extends MusicBeatState
{
	...
	//public var variables:Map<String, Dynamic> = new Map<String, Dynamic>(); Already defined at backend.MusicBeatState
	...
	{
		FlxG.stage.window.title = 'Friday Night Funkin\': Santiago Engine - ${SONG.song}';
	...
	inline private function createCountdownSprite(image:String, antialias:Bool):FlxSprite
	{
		...
		FlxTween.tween(spr, {y: spr.y + 1000}, Conductor.crochet / 1000, {
	...
	public dynamic function updateScore(miss:Bool = false)
	{
		...
		var tempScore:String = 'Score: ${songScore}' + (!instakillOnMiss ?  '   /   Accuracy: ${str}   \\   Misses: ${songMisses}' : "");
		scoreTxt.text = '${tempScore}\n';

		if (ClientPrefs.data.hideScore)
		{
	...
	{
		FlxTween.tween(iconP1.scale, {x: 1, y: 1}, 0.3 / playbackRate, {
		...
		FlxTween.tween(iconP2.scale, {x: 1, y: 1}, 0.3 / playbackRate, {
			ease: FlxEase.circOut,
			onUpdate: function(twn:FlxTween)
			{
				iconP2.updateHitbox();
	...
	{
		FlxG.stage.window.title = 'Friday Night Funkin\': Santiago Engine';
		#if LUA_ALLOWED
		for (lua in luaArray)
		{
			lua.call('onDestroy', []);
		...
		try
		{
			...
			if (newScript.exists('onCreate'))
			{
				...
				if (!callValue.succeeded)
				{
					...
					trace('[FAILED] failed to initialize tea interp!!! ($file)');
				}
				else
					trace('[SUCESS] initialized tea interp successfully: $file');
			}
		}
		catch (e)
		{
